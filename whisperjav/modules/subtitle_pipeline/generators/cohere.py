...
# The official (gated) repo. Finetune repos (e.g. efwkjn/cohere-asr-ja) ship
# only weights + configs — their config.json auto_map points at local
# modeling/processing modules WITHOUT shipping the .py files, so the remote
# code must be materialized from this base repo (see _ensure_remote_code).
_BASE_MODEL_ID = "CohereLabs/cohere-transcribe-03-2026"
...
class CohereTextGenerator:
    ...
    # Remote-code modules referenced by config.json's auto_map. The base
    # CohereLabs repo ships them; weights-only finetunes (efwkjn/cohere-asr-ja)
    # do NOT — they are copied in from the base snapshot by
    # _ensure_remote_code() before from_pretrained runs.
    _REMOTE_CODE_FILES = (
    ...
    # ------------------------------------------------------------------
    # Remote-code materialization for weights-only finetune repos
    # ------------------------------------------------------------------

    @classmethod
    def _has_remote_code(cls, snapshot_dir) -> bool:
        """True if the dir holds all remote-code .py modules (non-empty)."""
        from pathlib import Path
        snap = Path(snapshot_dir)
        try:
            return all((snap / name).stat().st_size > 0
                       for name in cls._REMOTE_CODE_FILES)
        except OSError:
            return False

    @classmethod
    def _find_base_code_dir(cls) -> Optional[str]:
        """Locate a cached base-repo snapshot holding the remote-code modules."""
        from pathlib import Path
        try:
            from huggingface_hub import snapshot_download
            path = snapshot_download(_BASE_MODEL_ID, local_files_only=True)
            if cls._has_remote_code(path):
                return path
        except Exception:
            pass
        # refs/main may point at a partial revision — scan all cached revisions.
        try:
            from huggingface_hub.constants import HF_HUB_CACHE
            snaps_dir = (Path(HF_HUB_CACHE)
                         / ("models--" + _BASE_MODEL_ID.replace("/", "--"))
                         / "snapshots")
            if snaps_dir.is_dir():
                for snap in sorted(
                    (d for d in snaps_dir.iterdir() if d.is_dir()),
    ...
    @classmethod
    def _ensure_remote_code(cls, target_dir: str) -> None:
        """
        Make sure target_dir contains the remote-code .py modules.

        Weights-only finetunes (e.g. efwkjn/cohere-asr-ja) publish config.json
        whose auto_map references local modules (modeling_cohere_asr etc.)
        without shipping the files — trust_remote_code loading fails with
        "Could not locate modeling_cohere_asr.py". The modules live in the
        base CohereLabs repo; copy them in from a cached base snapshot, or
        download just those files (needs the gated-repo token) as a fallback.
        """
        if cls._has_remote_code(target_dir):
            return
        import shutil
        src = cls._find_base_code_dir()
        if src:
            for name in cls._REMOTE_CODE_FILES:
                shutil.copy2(os.path.join(src, name),
                             os.path.join(target_dir, name))
            logger.info(
                "[CohereTextGenerator] Copied remote-code modules from base "
                "snapshot into finetune dir: %s -> %s", src, target_dir
            )
            return
        # No cached base code — fetch only the .py files from the gated base.
        cls._check_hf_access()
        try:
            from huggingface_hub import hf_hub_download
            for name in cls._REMOTE_CODE_FILES:
                got = hf_hub_download(_BASE_MODEL_ID, name)
                shutil.copy2(got, os.path.join(target_dir, name))
            logger.info(
                "[CohereTextGenerator] Downloaded remote-code modules from "
                "%s into finetune dir: %s", _BASE_MODEL_ID, target_dir
            )
        except Exception as exc:
            raise RuntimeError(
                "This Cohere finetune repo ships only weights — its modeling "
                "code lives in the gated base repo "
                f"({_BASE_MODEL_ID}), and fetching it failed.\n"
                "\n"
                f"  1. Visit {_MODEL_PAGE_URL} and click "
                "'Agree and access repository'.\n"
                "  2. Set HF_TOKEN (Read permission) in the environment.\n"
                "  3. Retry — only 4 small .py files are downloaded.\n"
                f"\nOriginal error: {exc}"
            ) from exc
    ...
    def load(self) -> None:
        ...
        if not local_path and not cached_snapshot:
            ...
            # The token/gating preflight applies to the gated base repo only —
            # finetune repos (e.g. efwkjn/cohere-asr-ja) are public.
            if cfg["model_id"] == _BASE_MODEL_ID:
            ...
            try:
                ...
                raise
            if cfg["model_id"] != _BASE_MODEL_ID:
                # Weights-only finetunes must be materialized to a local
                # snapshot dir FIRST so the base repo's remote-code modules
                # can be copied in before from_pretrained resolves auto_map
                # (loading straight from the Hub id would fail with
                # "Could not locate modeling_cohere_asr.py").
                try:
                    from huggingface_hub import snapshot_download
                    cached_snapshot = snapshot_download(cfg["model_id"])
                    logger.info(
                        "[CohereTextGenerator] Downloaded finetune snapshot: %s",
                        cached_snapshot,
                    )
                except Exception as exc:
        ...
        # Weights-only finetune dirs need the base repo's remote-code modules
        # copied in before trust_remote_code can resolve auto_map. No-op when
        # the .py files are already present (base repo, or a prior run).
