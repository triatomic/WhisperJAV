...
const TabManager = {
    ...
    switchTab(tabId) {
        ...
        // Hide main Run Controls when SRT Translate or File Manager tab is
        // active (translate has its own controls; file manager needs none)
        const runSection = document.querySelector('.run-section');
        if (runSection) {
            runSection.style.display = (tabId === 'tab4' || tabId === 'tab5') ? 'none' : '';
        }

        // Lazy-scan the model caches the first time File Manager is opened
        if (tabId === 'tab5') {
            FileManagerManager.load();
        }
    },
...
// ============================================================
// File Manager Tab (tab5) — downloaded model files per pipeline
// ============================================================
const FileManagerManager = {
    loaded: false,

    init() {
        document.getElementById('fileManagerRefreshBtn')?.addEventListener('click', () => this.load(true));
    },
    ...
    async load(force = false) {
        ...
        if (!container) return;
        container.innerHTML = '<p class="fm-placeholder">Scanning model caches…</p>';
        try {
            const result = await pywebview.api.get_model_inventory();
            if (!result || !result.success) {
                throw new Error((result && result.message) || 'scan failed');
            }
            this.render(result.groups || []);
            this.loaded = true;
        } catch (error) {
            container.innerHTML = '';
            const p = document.createElement('p');
            p.className = 'fm-placeholder';
            p.textContent = 'Failed to scan model caches: ' + error;
    ...
    render(groups) {
        ...
        if (!groups.length) {
            const p = document.createElement('p');
            p.className = 'fm-placeholder';
            p.textContent = 'No downloaded model files found — models download on first use of each pipeline.';
            container.appendChild(p);
            return;
        }
        ...
        groups.forEach(group => {
            ...
            title.className = 'fm-group-title';
            title.textContent = group.title + ' ';
            const total = document.createElement('span');
            total.className = 'fm-group-total';
            total.textContent = `(${group.items.length} item${group.items.length === 1 ? '' : 's'}, ${group.total})`;
            ...
            group.items.forEach(item => {
                ...
                btn.title = 'Open this location in File Explorer';
                btn.addEventListener('click', async () => {
                    try {
                        const r = await pywebview.api.open_model_path(item.path);
                        if (!r || !r.success) {
                            ErrorHandler.show('Error', (r && r.message) || 'Could not open path');
                        }
                    } catch (error) {
                        ErrorHandler.show('Error', 'Could not open path: ' + error);
                    }
                });
                row.appendChild(btn);
...
const EnsembleManager = {
    ...
    state: {
        ...
        pass2: {
            ...
            xxlExtraArgs: '--verbose True --standard_asia'  // Default XXL args (model flows via dedicated model field)
        },
    ...
    cohereModels: [
        ...
        // Weights-only decoder finetune (public); modeling code is auto-copied
        // from the cached base repo by the cohere generator's load().
        { value: 'efwkjn/cohere-asr-ja', label: 'Cohere-ASR-JA anime finetune    ~4-8GB (public)' }
    ],
    ...
    generateQwenModelTab(tabId, schemaSection, currentValues) {
        ...
        if (passState && passState.isAnimeWhisper) {
            ...
            modelOptions = [
                ...
                { value: 'efwkjn/cohere-asr-ja', label: 'efwkjn/cohere-asr-ja (JA/anime finetune, public, ~4-8GB VRAM)' },
            ];
...
const TranslatorManager = {
    ...
    // Tone-aware temperature defaults — mirrors _build_provider_options in
    // translate/service.py. Keep the two in sync.
    toneTemperatureDefaults: { standard: 0.5, contextual: 0.8, pornify: 1.2 },
    ...
    init() {
        ...
        // Tone change reflects the tone's default temperature into the Ollama
        // tuning field (an explicit field value would otherwise silently
        // override the backend's tone-aware default and pin pornify at the
        // standard temperature). User can still edit it afterwards.
        document.getElementById('translatorTone')?.addEventListener('change', (e) => {
            const temp = this.toneTemperatureDefaults[e.target.value];
            const tempInput = document.getElementById('translatorOllamaTemperature');
...
const TranslationSettingsModal = {
    ...
    init() {
        ...
        // so without this the tone-aware default (pornify=1.2, contextual=0.8)
        // never applied — the stale field value silently won. User edits after
        // the tone switch still stick (Save reads the field).
        document.getElementById('translationTone')?.addEventListener('change', (e) => {
            const temp = TranslatorManager.toneTemperatureDefaults[e.target.value];
